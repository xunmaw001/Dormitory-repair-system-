<template>
	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        ...
        <view class="search-form round">
				<input v-model="searchForm.weixiuyuanName" type="text" placeholder="维修员姓名"></input>
		</view>
...
<view class="uni-product-list" :style='{"borderRadius":0,"backgroundColor":"rgba(224, 224, 224, 1)"}'>
    <view
        ...
        <view class="uni-product-title"
            ...
            {{product.weixiuyuanName}}</view>
        <view class="image-view">
            <image :style='{"borderRadius":"0px","width":"100%","height":"100%"}' mode="aspectFill"
                   class="uni-product-image" :src="product.weixiuyuanPhoto?product.weixiuyuanPhoto.split(',')[0]:''"></image>
        </view>
        <view class="uni-product-price" v-if="product.weixiuyuanNewMoney != null"
              :style='{"fontSize":"24rpx","lineHeight":"60rpx","color":"#999","textAlign":"left"}'>
            <text class="">￥{{product.weixiuyuanNewMoney}}</text>
        </view>
        <view style="display: flex;justify-content: space-between;">
            <text v-if="isAuth('weixiuyuan','修改')" class="cuIcon-edit"
                  @click.stop="onUpdateTap(product.id)">修改</text>
            <text v-if="isAuth('weixiuyuan','删除')" class="cuIcon-delete"
            ...
            v-if="isAuth('weixiuyuan','新增')" class="add-btn" @click="onAddTap()">新增</button>
...
<script>
	export default {
		...
		methods: {
			//查询条件切换
			queryChange(e) {
				this.searchForm.weixiuyuanName = "";
			},
			//类别搜索
			...
			async search() {
				...
				if (this.searchForm.weixiuyuanName) {
                    searchForm['weixiuyuanName'] = this.searchForm.weixiuyuanName
                }
				if (this.searchForm.weixiuyuanName) {
					searchForm['weixiuyuanName'] = this.searchForm.weixiuyuanName
				}
