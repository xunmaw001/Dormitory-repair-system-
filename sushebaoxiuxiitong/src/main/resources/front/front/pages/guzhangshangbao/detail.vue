...
<script>
	export default {
		...
		methods: {
			...
			// 获取详情
			async init() {
				let res = await this.$api.info('guzhangshangbao', this.id);
				this.detail = res.data;
				// 主图
				this.swiperList = this.detail.guzhangshangbaoPhoto ? this.detail.guzhangshangbaoPhoto.split(",") : [];
			},
			...
			async upCallback(mescroll) {
				let res = await this.$api.list('guzhangshangbaoCommentback', {
					page: mescroll.num,
					limit: mescroll.size,
					guzhangshangbaoId: this.id
				});
				// 如果是第一页数据置空
			...
			// 添加评论
			async onCommentTap() {
                this.$utils.jump(`../guzhangshangbaoCommentback/add-or-update?guzhangshangbaoId=${this.id}`)
            },
		}
	}
</script>
