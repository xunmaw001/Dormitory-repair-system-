		...
		<form class="app-update-pv">
			<view
                ...
                <textarea
                        ...
                        v-model='ruleForm.guzhangshangbaoCommentbackText' placeholder="评价内容"></textarea>
            </view>
	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				let res = await this.$api.info(`guzhangshangbaoCommentback`, this.ruleForm.id);
		...
		methods: {
			...
			async onSubmitTap() {
			if ((!this.ruleForm.guzhangshangbaoCommentbackText)) {
				this.$utils.msg(`评价内容不能为空`);
				return
			}
				if (this.ruleForm.id) {
					await this.$api.update(`guzhangshangbaoCommentback`, this.ruleForm);
				} else {
					await this.$api.save(`guzhangshangbaoCommentback`, this.ruleForm);
				}
